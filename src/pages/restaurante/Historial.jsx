...
import { subscribeToRestaurantServices, getRestaurantByUserId, getRestaurant, subscribeToSettlements } from '../../services/firestore'
...
export default function RestauranteHistorial() {
  ...
  // Cargar datos del restaurante y suscribirse a servicios y liquidaciones
  useEffect(() => {
    const loadData = async () => {
      ...
      if (restaurant?.id) {
        ...
        const unsubSettlements = subscribeToSettlements((allSettlements) => {
          // Filtrar solo liquidaciones de este restaurante
          const restaurantSettlements = allSettlements.filter(s => 
            s.restaurantId === restaurant.id || 
            (s.type === 'restaurante' && s.entityId === restaurant.id)
          )
  ...
  // Servicios pendientes de liquidar (entregados y no settledRestaurant)
